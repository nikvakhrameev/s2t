...
- Calibrated 2026-09-22 on the 31 synthetic pairs of `scripts/eval_jev.py` against `jev-1.13.0`. Every targeted Noul separates the good pairs from its bad ones (good max / bad min): `content_added` 0.10 / 0.89, `content_dropped` 0.47 / 0.59, `polarity_flipped` 0.04 / 0.84, `details_changed` 0.12 / 0.90, `roles_swapped` 0.04 / 0.97, `responded` 0.11 / 0.93, `grammar_shifted` 0.29 / 0.92, `translated` 0.16 / 0.87 — so the 0.5 thresholds stay. The narrow one is `content_dropped`: a dropped «только» scores 0.59, a dropped self-correction («не хочу, то есть не могу» → «не могу», legitimate) 0.47. All 7 heuristic blind spots were caught (double negations 0.91 / 0.84, «15 30» → «16:30» 0.97, role and order swaps 0.97–0.98). The one-question alternatives do **not** separate (`meaning_changed` 0.37 / 0.34, `fidelity` 0.32 / 0.30, `edit_kind` 0.59 / 0.32) — all three miss the dropped «только», which is exactly the docs' warning about holistic questions; they stay out of the default set. A defect lights up several questions at once (a flipped negation also scores ~0.9 on `content_added`), which is fine with the max gate. Real runs: a verdict takes 310–420 ms on a warm connection; a 1-chunk dictation waits for all of it (+0.4 s), a 4-chunk one only for the last (+0.3 s in total — the pipelining works). ~1950 input tokens per request with the six questions ≈ $0.00008. The default `model` is pinned to `jev-1.13.0` (user decision): the thresholds belong to that version, an alias like `jev-latest` could move them silently. Would change it: false rejects in the journal (`jev_checks`) → raise that question's threshold; a new Jev version → rerun the eval, then change the pin.
