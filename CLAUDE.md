...
uv run python scripts/eval_jev.py [QUESTION...]             # calibrate the Jev guardrail on fixed good/bad pairs (needs the API key, no MLX)
...
- **Jev (`jev.py`, `cleanup.jev.*`, off by default) is the only networked code**: TypeSafe's cloud model judges each cleaned chunk against its raw text (DECISIONS D11). `mode: both` = heuristics, then Jev for what they let through; `mode: only` = Jev instead of the heuristics (only `empty` stays). One POST per chunk carries all active questions from the `QUESTIONS` catalog; each answer becomes a risk 0..1 compared with its threshold from `cleanup.jev.questions`; any question over its threshold rejects the chunk (`Rejection.reason = "jev"`, `words` = question ids) and the usual retry-by-sentence / raw fallback applies. Requests run on the judge's helper threads, never MLX: `LlmCleaner._clean_units` generates all units first and only then awaits verdicts, `Pipeline.run` calls `judge.warm()` up front so the TLS handshake (~0.5 s) overlaps STT, and `PendingVerdict` enforces a wall-clock deadline. No retries. Failures follow `on_error`. Raw `httpx` on purpose, not `typesafe-sdk`. Use the `typesafe:typesafe-ai` skill (live docs) before changing questions or the API call; keep the in-prompt examples different from the `scripts/eval_jev.py` pairs.
...
- `POST /transcribe` (multipart): `file` (any container ffmpeg reads), optional `language` (`auto|ru|en|…`), optional `cleanup` (bool). Response = `pipeline.Result`: `text`, `raw_text`, `language`, `audio_s`, `speech_s`, `no_speech`, `cleanup_used`, `cleanup_rejected_chunks`, `cleanup_rejections` (list of `{reason, words, raw, llm, fallback}`), `jev_checks` (list of `{raw, llm, risks, failed, ms, tokens, model, error}` — every Jev request, accepted chunks too), `hallucinations_dropped`, `timings_ms` (`jev` = how long verdicts held the result up; part of `cleanup`). Undecodable input → HTTP 400. `GET /health`.
